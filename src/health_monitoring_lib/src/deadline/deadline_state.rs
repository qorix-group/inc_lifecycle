...
impl DeadlineStateSnapshot {
    ...
    pub(super) fn set_running(&mut self) {
        self.0 = self.0 | DEADLINE_STATE_RUNNING;
    }

    pub(super) fn set_stopped(&mut self) {
        self.0 = self.0 | DEADLINE_STATE_STOPPED;
    }

    pub(super) fn set_underrun(&mut self) {
        self.0 = self.0 | DEADLINE_STATE_FINISHED_TOO_EARLY;
    }
}
    ...
    pub(super) fn update<F: FnMut(DeadlineStateSnapshot) -> Option<DeadlineStateSnapshot>>(
        ...
        self.0
            .fetch_update(Ordering::Relaxed, Ordering::Relaxed, |prev| {
                ...
                f(snapshot).and_then(|new_snapshot| Some(new_snapshot.as_u64()))
            })
            .map(|prev| DeadlineStateSnapshot::new(prev))
            .map_err(|prev| DeadlineStateSnapshot::new(prev))
    }
}

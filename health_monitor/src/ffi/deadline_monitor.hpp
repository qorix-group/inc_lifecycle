...
struct DeadlineMonitor
{
    ...
    Deadline get_deadline(Tag tag)
    {
        hm_Deadline *ffi_dl_ptr;
        // TODO: Error handling
        hm_dm_get_deadline(this->ptr, tag, &ffi_dl_ptr);

        return Deadline(ffi_dl_ptr);
    }

    Deadline create_custom_deadline(DurationRange range)
    {
        hm_Deadline *ffi_dl_ptr;
        // TODO: Error handling
        hm_dm_create_custom_deadline(this->ptr, range.min.count(), range.max.count(), &ffi_dl_ptr);
    ...
    void enable() {
        // TODO: Error handling
        hm_dm_enable(this->ptr);
    }

    void disable() {
        // TODO: Error handling
        hm_dm_disable(this->ptr);
    }
